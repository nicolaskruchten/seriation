...
pimage(shuffled, col=color, key=FALSE)
dev.off()
...
heatmap.2(shuffled, trace='none', col=color, key=FALSE, labRow = FALSE, labCol = FALSE,
          hclustfun = function(q) hclust(q, method="average") )
dev.off()

png("clustered_olo.png", width=outsize, height=outsize, units = 'px')
hmap(shuffled, method="OLO_average",col=color, key=FALSE,labRow = FALSE, labCol = FALSE)
dev.off()
